...
__all__ = ['get_humidity_by_id_from_path', 'get_humidity_by_id_from_query']

model = models.Humidity
crud_repo = crud.humidity
id_param_description = 'The UUID id of the humidity measurement'
...
async def get_humidity_by_id_from_path(
        id: Annotated[UUID, Path(description=id_param_description)]
) -> model:
    return await get(id=id)


async def get_humidity_by_id_from_query(
